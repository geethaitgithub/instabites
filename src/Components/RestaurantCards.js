...
const RestaurantCards = ({ cloudinaryImageId,
  name,
  cuisines,
  areaName,
  sla,
  ...
  return (
    <div className="w-60 h-70 p-2 m-4 space-x-3  rounded-lg shadow-xl bg-white hover:shadow-stone-300 transition ease-in-out delay-50 hover:scale-110 duration-200">
      
      <img className=' rounded-lg' src={IMG_CDN_URL+cloudinaryImageId} alt='RestaurantCardImg'></img>
      <h2 className="font-bold text-sm my-3 truncate">{name}</h2>
      <h5 className="text-sm font-xs tracking-wide truncate">
        {cuisines.join(", ")}
      </h5>
